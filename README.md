...
- プロジェクトごとのカテゴリー設定と、カテゴリー色によるプロジェクト・所属タスクの色分け
...
状態は `inbox / today / next / waiting / someday / done / cancelled` です。
...
- プロジェクトの詳細ではカテゴリー名と色を設定できます。同名カテゴリーの色はすべてのプロジェクトで統一されます。
